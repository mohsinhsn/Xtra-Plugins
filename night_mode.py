...
from main_startup import bot, Stylish, Config
...
async def scgrp(client, message):
    ...
    await pablo.edit(f"**Removed Chat {message.chat.title} With Id {message.chat.id} From Database. This Group Will Be No Longer Closed On 12Am(BST) And Will Opened On 06Am(BST)**")
    ...
    for warner in lol:
        try:
            await Stylish.send_message(
              int(warner.get("chat_id")), "`12:00 Am, Group Is Closing Till 6 Am. Night Mode Started !` \n**Powered By @StylishUser**"
            )
            await Stylish.set_chat_permissions(warner.get("chat_id"), ChatPermissions())
            async for member in Stylish.iter_chat_members(warner.get("chat_id")):
             if member.user.is_deleted:
                try:
                    await Stylish.kick_chat_member(warner.get("chat_id"), member.user.id)
                except:
                    pass
...
async def job_open():
    ...
    for warner in lol:
        try:
            ...
            await Stylish.set_chat_permissions(
                        warner.get("chat_id"),
                        ChatPermissions(
